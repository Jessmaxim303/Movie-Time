...
export class MovieDetail extends Component {
  ...
  render() {
    ...
    const backgroundStyling = {
      background: `linear-gradient(to top, rgba(0, 0, 0), rgba(0, 0, 0, 40%)), url(${backdrop_path}) no-repeat center top`,
      ...
      <section className='movie-detail' style={backgroundStyling} key={id}>
          ...
          : null}
            <p className='movie-detail__overview'>{overview}</p>
              <div className="movie-nav_buttons">
                <button className='movie-detail-button watch-now'>WATCH NOW</button>
                <button className='movie-detail-button movie-favorite'>FAVORITE</button>
              </div>
          </div>
        </section>
      </div>
